...
ARH_NLQ <- function(burn = 200, n = 50, t = seq(0, 1, l = 101), c = 0.5, X0) {
  
    phi <- function(t, s) (c/0.7468) * exp(-((t^2 + s^2) / 2)) 
    ...
    for (i in 2:(n + burn)) {
      ...
      for (j in 1:length(t)) {
        ...
        X[i, j] <- (sum(phi(t[j], t) * (X[i - 1, ])^2) / (length(t) - 1)) + bb[j]
      }
    }
...
for (j in 1:length(n)) {
  for (m in 1:MC) {
    ...
    # ARH NLQ generation
    X_NLQ <- ARH_NLQ(burn = burn, n = n[j], t = t, X0 = BrowBridge(t), c = 0.5)
    X_NLQ <- list("fstoch_proc" = X_NLQ[(burn + 1):(burn + n[j])])
    
    # STAGE 1: Is X_n an ARH(z) process?
    cat(paste0("\nSample size ", n[j], ", iteration ", m))
    testing_ARHz <- ARHz_test(X_NLQ, z = z_null, B = B, hyp_simp = FALSE,
